  ...
  <body>
    <header class="clearfix">
      ...
      <h1>Branch Liaison Management Information System</h1>
         <p>  Payment Receipt (<?php if(isset($payment->pp_txn_ref_no) && $payment->pp_txn_ref_no!=""){ echo 'Manual';}else{echo 'Online';} ?>)</p>
      ...
      <div id="project">
        ...
        <div><span>Transaction ID</span> <?php if(isset($payment->pp_txn_ref_no) && $payment->pp_txn_ref_no!=""){ echo $payment->pp_txn_ref_no;}else if(isset($payment->pp_retreival_ref_no)){echo $payment->pp_retreival_ref_no;} ?></div>
        
        <div><span>Transaction Date</span><?php if(isset($payment->updated_at)){ echo $payment->updated_at;}else if(isset($payment->created_at)){ echo $payment->created_at ;} ?></div>
    ...
    <main>
      <table>
        ...
        <tbody>
          <tr>
            <td class="service"><?php $service = collect(App\Lookups\ServiceType::DATA)->firstWhere('value', $branch->service_type_id); echo $service['text']; ?> (Renewal)</td>
            <td class="desc">$<?php if(isset($payment->usd_amount)){ echo $payment->usd_amount;} ?></td>
            <td class="unit"><?php if(isset($payment->pkr_rate)){ echo $payment->pkr_rate;} ?></td>
            <td class="total"><?php if(isset($payment->usd_amount)){ echo $payment->usd_amount * $payment->pkr_rate;} ?></td>
          </tr>
          <!-- <tr>
