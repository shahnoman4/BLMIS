...
export default class BranchTopTenSectors extends React.Component{
    ...
    componentDidMount(){
        ...
        busy.apps = true;
        this.setState({busy});
        api.get('/report/branchtoptensectors')
             .then(response=>{
                    busy.apps = false;
                    data.apps = response.all;
                    graph.labels = response.sector;
                              ...
                              <tbody>

                                   {
                                      this.state.reports &&  this.state.reports.map((report,index)=>{
                                            return(
                                                    <tr key={index}>
                                                      <td>{__app.LOOKUP.text('Sector',report.sector_id)}</td>
                                                      <td>{report.total}</td>
